...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mDbHelper = new WordsDBhelper(this,null,null,1);
    ...
    public boolean onContextItemSelected(final MenuItem item) {
        ...
        switch (item.getItemId()){
            case R.id.action_delete:
                ...
                textId.setOnCreateContextMenuListener( this );
          ...
          /*  case R.id.action_tj:
                ...
                textId =(TextView)itemView.findViewById(R.id.textId);
                textId.setOnCreateContextMenuListener( this );
                final String strId0  = textId.getText() .toString() ;
                textId.setOnClickListener( new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        Intent intent = new Intent(MainActivity.this,ScbActivity.class);
                        intent.putExtra("extra_data",strId0);
                        startActivity(intent);
                    }
                } );
    ...
    //使用sql语句添加到生词本
   /* private void CopyUserSql(String strId) {
        String sql="insert into scb where _id='"+strId+"'";
        //Gets the data repository in write mode
        SQLiteDatabase db = mDbHelper.getReadableDatabase();
        db.execSQL(sql);
    }
    添加到生词本对话框
    private void CopyDialog(final String strId){
        new AlertDialog.Builder(this).setTitle("添加到生词本").setMessage("是否添加到生词本?").setPositiveButton("确定", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
                //既可以使用Sql语句添加，也可以使用使用add方法添加
                CopyUserSql(strId);
                //Delete(strId);
                setWordsListView(getAll());
                ArrayList<Map<String, String>> items=getAll();
